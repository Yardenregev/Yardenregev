...
const addNewBookmark = (bookmarks, bookmark) => {
  ...
  setElementAttribute("play", onPlay, controlsElement);
  setElementAttribute("delete", onDelete, controlsElement);
...
const viewBookmarks = (currentBookmarks=[]) => {
  ...
  const recordPanelElement = document.createElement('div')
  recordPanelElement.id = "record-pannel"
  setElementAttribute("record",onRecord,recordPanelElement)
  ...
  if (currentBookmarks.length > 0) {
    ...
    containerElement.innerHTML = '<div class="title">This is the control pannel.</div>';
    containerElement.appendChild(recordPanelElement);
...
const onRecord = async e =>{
  ...
  console.log("record")
  const recordPanelElement = document.getElementById("record-pannel");
  removeElementAttribute("record", recordPanelElement);
  setElementAttribute("stop",onStop,recordPanelElement);
}
...
const onStop = async e =>{
  ...
  console.log("stop")
  const recordPanelElement = document.getElementById("record-pannel");
  removeElementAttribute("stop", recordPanelElement);
  setElementAttribute("record",onRecord,recordPanelElement);
}
...
const setElementAttribute =  (src, eventListener, controlParentElement) => {
  const controlElement = document.createElement("img");

  controlElement.src = "assets/" + src + ".png";
  controlElement.title = src;
  controlElement.id = src + "-control"
  controlElement.addEventListener("click", eventListener);
  controlParentElement.appendChild(controlElement);
};

const removeElementAttribute = (src, controlParentElement) =>{
  const elementToRemove = document.getElementById(src+"-control");
  controlParentElement.removeChild(elementToRemove);
}
